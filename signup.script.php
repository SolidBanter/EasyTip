...
if (isset($_POST['bus-signup-submit'])) {
    ...
    $cardNum = mysqli_real_escape_string($conn, $_POST['user-signup-card-number']);
        ...
        if ($pass != $conf_pass) {
            ...
            if (!filter_var($email, FILTER_VALIDATE_EMAIL)) {
                ...
                if ($emailCheckResultCheck > 0) {
                    ...
                    $hashedPassword = password_hash($pass, PASSWORD_DEFAULT);
                    $user_id = uniqid('', true);
                    $insertUser = "INSERT INTO users (id, name, pwd, email,cardNum) VALUES ('$user_id', '$name', '$hashedPassword', '$email','$cardNum')";
                    if (mysqli_query($conn, $insertUser)) {
